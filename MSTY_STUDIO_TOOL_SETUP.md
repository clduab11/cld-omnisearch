...
When you click **"New Tool"** in Msty Studio, paste this JSON
configuration:
...
1. Click the asterisk (*) icon next to `cld-omnisearch` in your tool
   list
...
5. Add notes: "Complete web search, AI search, and content processing
   toolkit"
6. Click **"Add"**

### 6. Enable Auto-Approval (Safe for All Tools)

Since all cld-omnisearch tools are **read-only** (they only search and
retrieve data, never modify anything), you can safely enable
auto-approval:
...
**Note:** Msty Studio handles approval settings through the UI rather
than JSON configuration. The `alwaysAllow` property is not used in the
JSON schema.
...
- **GitHub**: Free personal access tokens at
...
The AI should automatically use the `web_search` tool and return
results.
...
- Some models don't support tool calling well - try Claude 3.5 Sonnet
  or GPT-4
...
- **Package Issues**:
  https://github.com/clduab11/cld-omnisearch/issues
